...
public class SynchronizedExam {
	public static void main(String[] args) {
		...
		CustomerThread th2 = new CustomerThread(bank , "입출자" , false);
...
class Bank{
	...
	public synchronized void balanceChange(String name, boolean state) {
		if(state) {//입금
			...
			balance++;
			System.out.println(name+"=> 증가후 현재 잔액 : "+balance);
			...
			System.out.print(name+"=> 현재 잔액 : "+balance+" / ");
			balance--;
			System.out.println(name+"=> 감소후 현재 잔액 : "+balance);
		}
		
		notifyAll();//wait()에 의해 대기중인 스레드를 깨운다.
	}
}
