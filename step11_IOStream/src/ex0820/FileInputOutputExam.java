package ex0820;

import java.io.FileInputStream;
import java.io.FileOutputStream;
import java.io.IOException;

public class FileInputOutputExam {
	public FileInputOutputExam() {
		FileInputStream fis = null;
		FileOutputStream fos = null;
		try {
			
			fis = new FileInputStream("src/ex0820/a.txt");
			/*while(true) {
			int i = fis.read();
			if(i==-1)break;
			System.out.println(i+" = " (char)i);
			}*/
			
			
			int len = fis.available();//읽을수 있는 byte수 반환.
			System.out.println("len = "+ len);
			
			//byte [] 크기 만큼 한번에 읽기.
			byte [] b = new byte [len];//메모장에 있는 단어의 byte를 위에서 구하고 그 변수를 넣어 출력
			fis.read(b);
			//byte[] 을 String으로 변환 하는 방법
			String str = new String(b);
			System.out.println(str);
			System.out.println("**완료**");
			
			//파일에 내용을 저장=쓰기
			fos = new FileOutputStream("src/ex0820/save.txt");
			//fos = new FileOutputStream("src/ex0820/save.txt", true);
			//true는 덮어쓰지않고 이어쓴다는 의미. 즉 아래의 fos의 모든 값이 반복되어 출력된다는 것.
			/*fos.write(65);//A
			fos.write(66);//B
			fos.write(67);//C
			
			fos.write(13);//줄바꿈
			fos.write(10);//줄바꿈
			
			fos.write(70);//F
			
			//String ->byte[]로 변환 ->data.getBytes()
			String data = "집에 가고 싶어요~";
			fos.write(data.getBytes());*/
			
			fos.write(b);
			//b는 a.txt를 읽은 값. 그값을 읽어서 출력하는 것이기때문에 이것은 copy에 해당한다.
			
			
	}catch(IOException e) {
		e.printStackTrace();
		
	}finally {
		try {
		if(fis!=null) fis.close();
		if(fos!=null) fos.close();
	}catch(IOException e) {
		e.printStackTrace();
	}
	}
	}
	
	public static void main(String[] args) {
		new FileInputOutputExam();
	}
}
